...
class usuario
{
    ...
    private function loguear() {
        ...
        if (self::autenticar($correo, $contrasenia)) {
            self::actualizarUltimaConexion($ultimaconexion);
            $usuarioBD = self::obtenerUsuarioPorCorreo($correo);
    ...
    private function actualizarUltimaConexion($ultimaconexion)
    {
        try {
            $comando = "UPDATE " .
                self::NOMBRE_TABLA .
            ...
            if ($resultado) {
                return self::ESTADO_CONEXION_ACTUALIZADA;
            } else {
                return self::ESTADO_FALLA_DESCONOCIDA;
            }
        } catch (PDOException $e) {
            throw new ExcepcionApi(self::ESTADO_ERROR_BD, $e->getMessage());
        }
